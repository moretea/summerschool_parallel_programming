...
class Canvas:
    ...
    def blit(self, other, atx, aty):
        for y in xrange(other.height):
            io = ((other.height - y - 1) * other.width) * 3
            il = ((self.height - (aty + y) - 1) * self.width + atx) * 3
            self.bytes[il:il+3*other.width] = other.bytes[io:io+3*other.width]
    ...
    def save(self):
        with open(self.filenameBase + '.ppm', 'wb') as f:
            # print "Saving %s.ppm..." % self.filenameBase
...
class Scene:
    ...
    def render(self, canvas, basex, basey, totalwidth, totalheight):
        ...
        width = halfWidth * 2
        height = halfHeight * 2
        pixelWidth = width / (totalwidth - 1)
        pixelHeight = height / (totalheight - 1)
        ...
        for y in range(canvas.height):
            ...
            for x in range(canvas.width):
                xcomp = vpRight.scale((x + basex) * pixelWidth - halfWidth)
                ycomp = vpUp.scale((y + basey) * pixelHeight - halfHeight)
...
if __name__ == '__main__':
    ...
    c1 = Canvas(320,480,'test_raytrace_big1')
    c2 = Canvas(320,480,'test_raytrace_big2')
    s = make_demo_scene()
    s.render(c1, 0, 0, 640, 480)
    c1.save()
    s.render(c2, 320, 0, 640, 480)
    c2.save()
